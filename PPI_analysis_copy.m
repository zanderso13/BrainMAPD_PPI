...
basedir = '/Users/zaz3744/Documents/current_projects/ACNlab/BrainMAPD/MID_func_conn/first_levels/ppi';
...
%exclusions = [corrupt_data,motion_problems,admin_problems,grubbs_outlier];
 exclusions = {'10001','10084','10094','10102','10125','10140'...
    ...
    '20674','21111','21178','21223','21597','21386','20877',...
    '10272','10319'}; % 20877 is missing specific trial types in one of the runs. 10272 only has one run
...
for roi = 1:length(roidir)
    ...
    con1run1 = filenames(fullfile('*/ses-2/consumption/run-1/con_0001.nii'));
    con2run1 = filenames(fullfile('*/ses-2/consumption/run-1/con_0002.nii'));
    
    con1run2 = filenames(fullfile('*/ses-2/consumption/run-2/con_0001.nii'));
    con2run2 = filenames(fullfile('*/ses-2/consumption/run-2/con_0002.nii'));
...
%     site_regressor = array2table(site_regressor); site_regressor.Properties.VariableNames = {'site'};
    ...
    regout1.(roidir{roi}) = regress(acon1.(roidir{roi}),[0.001, 'unc']);
    regout2.(roidir{roi}) = regress(acon2.(roidir{roi}),[0.001, 'unc']);
    ...
    s2s(isoutlier(s2s.predict_ofc,"gesd"),:)=[];
    s2s(isoutlier(s2s.predict_amyg,"gesd"),:)=[];

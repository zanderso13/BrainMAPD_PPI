...
%
% :Usage:
%       [nuis_matrix, n_spike_regs, n_spike_regs_percent] = make_nuisance_covs_from_fmriprep_output2020(fmriprep_confounds_fname, TR, FD_spike_cutoff, spike_additional_vols, ndummies, fast_TR, varargin)
%
...
% 2) spikes for DVARS z-score > 3. If fast_TR = true, disabled -- DVARS not
% a good indicator of motion in fastTR data (Power 2019)
...
%
%   **ndummies:**
%        how many initial vols to drop from the cov matrix. 
%   These volumes should get dropped from fMRI data elsewhere)
%
%   **fastTR:**
%        0 or 1. Whether to modify spike regressor calculation according to
...
function [nuis_matrix, n_spike_regs, n_spike_regs_percent] = make_nuisance_covs(fmriprep_confounds_fname, TR, FD_spike_cutoff, spike_additional_vols, ndummies, fast_TR, varargin)

if FD_spike_cutoff < .2 || FD_spike_cutoff > .7
    warning('Atypical FD spike cutoff chosen. See documentation');
end

gs = false; wm = false; csf = false; aCompCor = false; tCompCor = false; antpost = false;
% if any(strcmp(varargin, 'gs')), gs = true; end
if any(strcmp(varargin, 'wm')), wm = true; end
if any(strcmp(varargin, 'csf')), csf = true; end
% if any(strcmp(varargin, 'aCompCor')), aCompCor = true; end
% if any(strcmp(varargin, 'tCompCor')), tCompCor = true; end
...
% hack for OLP4CBP study. sorry. remove later. Yoni Ashar, July 2020
if antpost, nuis_matrix = [nuis_matrix zscore(R{:, {'anterior_edge', 'posterior_edge'}})]; end
...
fprintf('Num FD-spikes:\t%d\nNum DVARS-spikes:\t%d\nNum addtnl spikes:\t%d\n', sum(FD_spikes), sum(dvars_spikes), length(allspikes) - sum(FD_spikes) - sum(dvars_spikes))

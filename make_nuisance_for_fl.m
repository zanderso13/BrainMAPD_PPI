...
function [Rfull, Rselected, n_spike_regs, framewise_displacement_final, gsr_final] = make_nuisance_for_fl(fmriprep_confounds_fname, raw_img_fname, TR, spike_additional_vols, confound_savedir, preproc_dir)
...
Rselected = R(:,spike_cols | dvars_cols | additional_spike_cols); %| additional_spike_cols);
motion_final = [R.trans_x,R.trans_y,R.trans_z,R.rot_x,R.rot_y,R.rot_z,R.trans_x_derivative1,R.trans_y_derivative1,R.trans_z_derivative1,R.rot_x_derivative1,R.rot_y_derivative1,R.rot_z_derivative1];
...
Rselected = [motion_final,csf_final,white_matter_final,Rselected];
...
% write back to file
% confound_savedir = fullfile(preproc_dir, strcat(PID), strcat('ses-',num2str(ses)), 'func', '*task-MID*confounds*.tsv');
% writetable(Rselected, confound_savedir, 'filetype', 'text');
